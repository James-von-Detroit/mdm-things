...
/usr/bin/touch "/Volumes/${SELECTEDVOLUME}"/computer_name.txt
echo $COMPUTER_NAME > "/Volumes/${SELECTEDVOLUME}"/computer_name.txt

echo
echo "Installing macOS to /Volumes/${SELECTEDVOLUME}..."

# build our startosinstall command
CMD="\"${STARTOSINSTALL}\" --pidtosignal $$ --agreetolicense --volume \"/Volumes/${SELECTEDVOLUME}\""
